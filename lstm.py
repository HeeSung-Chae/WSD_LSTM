...
for posDir in posSentGlob:
    ...
    if posDir.count('mok') > 0:
        ...
        for sentence in posList:
            ...
            # tempSentVec = ""
            # eojul = sentence.split(' ')
            # # print("sentence : ", sentence)
            # for word in eojul:
            #     # word는 eojul에 각 단어들
            #     # vecDic[vecDicNum]은 각 word의 벡터
            #     for vecDicNum in vecDic:
            #         if word == vecDicNum:
            #             tempSentVec += vecDic[vecDicNum] + "\t"
            #             # print(word, vecDic[vecDicNum])
            #             # break
            #     # print(t, vecDic[t])
            # # break
            # sentVecDic[sentence] = tempSentVec

        # 문장 출력 확인용, a=sentence
        # for a in posList:
        #     print(a, end='')
        #     for b in sentVecDic:
        #         if a == b:
        #             print(sentVecDic[a])
        #             # for c in sentVecDic[a]:
        #             #     asdf += 1
        #             #     print(c)
        #     print()

        # 문장 벡터 갯수 확인용
        # a는 문장, tempC1는 어절 갯수, tempC2는 총 차원수(단어가 2개면 2*300=600)
        # print(sentVecDic.__len__())
        # for a in sentVecDic:
        #     print(a, end=', ')
        #     tempC1 = 0
        #     tempC2 = 0
        #     wordLen = 0
        #     for b in a.split(' ')[0:-1]:
        #         for c in vecDic:
        #             if c == b:
        #                 for d in vecDic[c]:
        #                     tempC2 += 1
        ...
        for sentence_content in posList:
            ...
            # train = tf.train.RMSPropOptimizer(learning_rate=0.002).minimize(loss)
            # train = tf.train.GradientDescentOptimizer(learning_rate=0.002).minimize(loss)
            # train = tf.train.AdadeltaOptimizer(learning_rate=0.002).minimize(loss)
            # train = tf.train.ProximalGradientDescentOptimizer(learning_rate=0.002).minimize(loss)
